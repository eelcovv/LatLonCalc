...
from LatLon.lat_lon import (LatLon, string2latlon, Latitude, Longitude)
...
def test_LatLon_tostring():
    ...
    assert_equal(palmyra.to_string('d%, %m%, %S%, %H', n_digits_seconds=2), ('5, 52, 59.88, N', '162, 4, 59.88, W'))
    # to_string method with fancy separators:
    assert_equal(palmyra.to_string('H%_%d%deg %M%"', n_digits_decimal_minutes=3), ('N_5deg 52.998"', 'W_162deg 4.998"'))
...
def test_LatLon_fromstring():
    ...
    assert_almost_equal(palmyra.lat.decimal_degree,
                        Latitude(5.8833).decimal_degree)  # Failure to convert from hemisphere, degree, minutes string
